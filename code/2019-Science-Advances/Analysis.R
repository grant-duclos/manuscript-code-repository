...
# n.states: integer, expected number of gene sets or cell clusters

# n.starts: integer, number of random starts for LDA

# seed.start: integer; seed for random number generation used during LDA

# max.q: maximum acceptable FDR corrected p-value

# min.coef: minimum acceptable regression coefficient

# min.spec: minimum acceptable state specificity (value between 0 and 1)

# min.sim: minimum acceptable state similaririty (value between 0 and 1)
...
run_analysis <- function(
	...
	# Extract Topics Matrix from LDA output: Topics = States
	...
	max.it <- 100
	cat("Begin Negative Binomial Regression \n")
	...
	if (type == "genes") {
		...
		for (k in 1:ncol(states)) {
			...
			result.stats <- data.frame(row.names=colnames(counts.mat))
			result.stats[unlist(lapply(1:ncol(states), paste, c("p", "FDR q", "Coef", "OR", "Spec", "Sim")))] <- NA
			for (k in 1:ncol(states)) {
				cat("State", k, sep=" ", "\n")
	...
	cat("Negative Binomial Regression Complete \n")
	...
	cat("State-Specificity Complete \n")
	...
	cat("State-Similarity Complete \n")

	# Feature Selection
	# Include State Assignment in results matrix
